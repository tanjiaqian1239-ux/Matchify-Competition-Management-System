    ...
    if ($action === 'approve') {
        ...
        if ($judge) {
            ...
            if ($existing) {
                ...
                $upd_pass = $conn->prepare("UPDATE users SET password=?, status='active' WHERE id=?");
                $upd_pass->bind_param("si", $hashed_pass, $user_id);
                $upd_pass->execute();
            } else {
            ...
            $upd = $conn->prepare("
                ...
                SET status='approved', judge_id=?, temp_password=? 
                WHERE id=?
            ");
            ...
            try {
                ...
                $mail->Subject = "You are approved as a Judge - Matchify Competition Management Platform";
                  ...
                  <div style='max-width:520px; margin:auto; background:#fff; border-radius:16px; overflow:hidden; box-shadow:0 8px 30px rgba(0,0,0,0.08);'>
                    ...
                    <div style='padding:30px;'>
                      ...
                      <p style='color:#4b5563; font-size:14px; line-height:1.7;'>
                        Congratulations! You have been approved as a <b>Judge</b> on Matchify Competition Management Platform.
                      </p>

                      <div style='background:#f5f3ff; border:1px solid #ddd6fe; border-radius:12px; padding:20px; margin:20px 0;'>
                        <p style='margin:0 0 10px; font-size:13px; color:#6b7280; font-weight:600; text-transform:uppercase; letter-spacing:0.5px;'>Your Login Credentials</p>
                      ...
                      <div style='text-align:center; margin:24px 0 10px;'>
                        <a href='$login_url'
                           style='display:inline-block; padding:14px 36px; background:linear-gradient(135deg,#6d5dfc,#8a7dff); color:#fff; border-radius:30px; text-decoration:none; font-weight:600; font-size:14px;'>
                          Login to Matchify
                        </a>
                      </div>
                    </div>
                    ...
                    <div style='background:#f9fafb; padding:16px; text-align:center; border-top:1px solid #f0f0f0;'>
                      <p style='margin:0; font-size:12px; color:#9ca3af;'>This is an automated email from Matchify Competition Management Platform. Please do not reply.</p>
                    </div>
                ...
                $mail->AltBody = "Hello $name, you are approved as judge.\nEmail: $email\nTemp Password: $temp_pass\nLogin: $login_url";
...
/* ================= COMPETITION ================= */
$comp = $conn->prepare("SELECT * FROM competition_applications WHERE id=?");
$comp->bind_param("i", $comp_id);
$comp->execute();
$comp = $comp->get_result()->fetch_assoc();
...
<!-- OVERLAY & POPUP — outside .container -->
...
function openPopup(id) {
    ...
    popup.offsetHeight;
    popup.style.animation = "pop 0.2s ease";
}
